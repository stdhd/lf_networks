...
from utilities.logging import make_nn_var_plot
...
class FVD(Metric):
    ...
    def __init__(self,n_samples):
        ...
        self.model_path = 'data/i3d_kinetics_rgb.pth'
        if 'DATAPATH' in os.environ:
            print("####################PATH IS", os.environ['DATAPATH'])
            self.model_path = os.path.join(os.environ['DATAPATH'],'i3d_kinetics_rgb.pth')
...
class LPIPS(Metric):
    ...
    def __init__(self, input_format="float-1"):
        ...
        self.lpips_net = lpips_net(net='alex').eval()
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        with torch.no_grad():
            lpips_batch = self.lpips_net(target,preds).cpu()
...
def calculate_FVD(model, data_gen, data_orig, batch_size, cuda=True):
    ...
    del data_gen
    del data_orig
    array_sum = np.sum(m1) + np.sum(s1) + np.sum(m2) + np.sum(s2)
