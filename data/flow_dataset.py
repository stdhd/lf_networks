...
from utilities.general import LoggingParent,preprocess_image
from data.base_dataset import BaseDataset

class MovingMnistDataset(BaseDataset):
    def __init__(self, transforms, datakeys, config, train=True, google_imgs=False, debug=False):
...
class PlantDataset(BaseDataset):
    def __init__(self, transforms, datakeys, config, train=True, google_imgs=False, debug=False):
        ...
        self.logger.info(f"Initializing {self.__class__.__name__}.")
        self.debug = debug
        #self.data['sample_groups'] = torch.from_numpy(
        #    np.load(os.path.join(config['data_root'], 'mnist_test_seq.npy')).swapaxes(0, 1)[:-self.split]
        #)
        ...
        if "max_fid" not in self.data and not isinstance(self, SharedDataset):
        ...
        if not isinstance(self,SharedDataset) and not isinstance(self,IperDataset):
        ...
        elif isinstance(self,IperDataset):
        ...
        #self.logger.info(
        #    f'Initialized {self.__class__.__name__} in "{msg}"-mode. Dataset consists of {self.__len__()} samples. ' + flow_msg
        #)
    ...
    def _set_instance_specific_values(self):
        ...
        self.min_frames = 5
        self.obj_weighting = self.config['object_weighting'] if 'object_weighting' in self.config else True
...
class SharedDataset(PlantDataset):
    def _set_instance_specific_values(self):
        assert self.datakeys == ["images"], "SharedDataset only valid for training image autoencoders..."
        self.datapath = ""
        self.metafilename = "/export/scratch/compvis/datasets/shuffle_poke_data/meta_data_dataset_weights"
...
class VegetationDataset(PlantDataset):
    ...
    def _make_split(self,data):
        split_data = {"train":{},"BairDatasettest":{}}
        train_ids = np.flatnonzero(data["train"])
        test_ids = np.flatnonzero(np.logical_not(data["train"]))
        assert np.intersect1d(train_ids,test_ids).size == 0
        split_data["train"] = {
...
class BairDataset(VegetationDataset):
    ...
    def _set_instance_specific_values(self):
        ...
        self.datapath = "/export/scratch/compvis/datasets/bair/"
...
class Human36mDataset(PlantDataset):
    ...
    def _make_split(self,data):
        ...
        #     key = "actor_id"
        # elif self.split == "official":
        #     train_indices = np.flatnonzero(np.isin(data["actor_id"],[1,5,6,7,8]))
        #     test_indices = np.flatnonzero(np.isin(data["actor_id"],[9,11]))
        #
        #     split_data["train"] = {
        #         key: data[key][train_indices] for key in data
        #     }
...
if __name__ == "__main__":
    ...
    from utilities.logging import make_flow_video_with_samples
    from utilities.logging import make_flow_img_grid
    from utilities.general import  parallel_data_prefetch
    ...
    if config["data"]["dataset"] in ["ImageNet", "SharedDataset"]:
        only_images = True
        datakeys = ["images"] #,"flow", "poke", "poke_coords"]
    else:
        only_images = False
