...
from utilities.general import preprocess_image,get_logger
...
class BaseDataset(Dataset):
    def __init__(self, transforms, datakeys: list, config: dict, train=True):
        ...
        self.fix_n_pokes = 'fix_n_pokes' in self.config and self.config['fix_n_pokes']
        if self.fix_n_pokes:
            pass
    ...
    def __getitem__(self, idx):
        ...
        # Determine sample id corresponding the given video id (idx)
        #print('flatnonzero ', np.flatnonzero(self.datadict["vid"] == self.datadict["vid"][idx[0]]))
        sidx = int(np.random.choice(np.flatnonzero(self.datadict["vid"] == self.datadict["vid"][idx[0]]), 1))

        # Choose randomly another sample id which is not part of the chosen video at index idx, this is called the transfer video
    ...
    def _get_valid_ids(self,length,index = None):
        ...
        if self.yield_videos:
            if self.var_sequence_length:
                ...
                #print('start, max', start_id, max_id_fid)
    ...
    def _get_mask(self,ids):
        ...
        if self.filter_flow or self.fancy_aug:# or self.flow_weights:
    ...
    def _get_yield_ids(self,ids):
        ...
        if self.yield_videos:
            if ids[-1] == -1:
                if self.var_sequence_length:
                    #print('optiona', self.min_frames, self.outside_length)
                    n_frames = self.min_frames + self.outside_length
                    yield_ids = np.stack([start_id]* n_frames,axis=0).tolist()
                else:
                    #print('optionb', self.max_frames)
                    yield_ids = np.stack([start_id]* (self.max_frames+1),axis=0).tolist()
            else:
                #print('optiond', start_id, self.min_frames, ids[-1], self.subsample_step, self.max_frames)
            ...
            #print('optionc', ids[-1], start_id, self.valid_lags[0], self.subsample_step)
            if ids[-1] == -1:
