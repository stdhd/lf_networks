...
def make_flow_img_grid(start_img,tgt,samples,poke,flow, image_range="float-1",poke_normalized=False,flow_original=None):
    ...
    poke_img_row = cv2.UMat.get(cv2.putText(cv2.UMat(poke_img_row), "Pokes and Sources", (int(poke_img_row.shape[1] // 3), poke_img_row.shape[0] - int(poke_img_row.shape[0] / 6)), cv2.FONT_HERSHEY_SIMPLEX,
...
def make_video(targets, preds, n_logged, log_wandb=True, n_max_per_row=10, show_frames=True, bair = False):
    n_logged = min(n_logged, n_max_per_row)
    if not bair:
        ...
        src =   ((preds.permute(0, 1, 3, 4, 2).detach().cpu().numpy()) * 255.0 ).astype(np.uint8)[:n_logged]
        tgt = ((targets.permute(0, 1, 3, 4, 2).detach().cpu().numpy()) * 255.0).astype(np.uint8)[:n_logged]
...
def make_pq_diagram(p, q):
    try:
        fig, axs = plt.subplots(2)
        for i in range(p.size(0)):
            p_out = p[i].reshape(p.size(1), -1)
            pU, pS, pV = torch.pca_lowrank(p_out)
            pReprojected = torch.matmul(p_out, pV[:, :1])

            q_out = q[i].reshape(q.size(1), -1)
            qU, qS, qV = torch.pca_lowrank(q_out)
            qReprojected = torch.matmul(q_out, qV[:, :1])
            axs[0].scatter(pReprojected.cpu(), qReprojected.cpu(), marker='.', linewidths=[0.5, 0.5], alpha=0.3)
            axs[1].scatter(p.cpu(), q.cpu(), marker='.', linewidths=[0.5, 0.5], alpha=0.4)
        axs[0].set_title('Principal components of p, q states')
        axs[1].set_title('All p, q states')
        plt.tight_layout()

    except Exception as ex:
        print(ex)
    finally:
        output_image = wandb.Image(plt)
        plt.close()
        return output_image


def make_video_second_fix(targets, n_logged, log_wandb=True, n_max_per_row=10, show_frames=True, bair = False):
    n_logged = min(n_logged, n_max_per_row)
    if not bair:
        tgt = ((targets.permute(0, 1, 3, 4, 2).detach().cpu().numpy() + 1.) * 127.5).astype(np.uint8)[:n_logged]
    else:
    ...
    for tgt_row in tgt_rows:
        tgt_row = put_text_to_video_row(tgt_row,"Sampled videos",display_frame_nr=show_frames)
        sub_grids.append(np.concatenate([tgt_row],axis=1))
...
def save_video(video,savepath,fps= 2):
