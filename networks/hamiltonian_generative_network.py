...
class HGN(nn.Module):
    ...
    def __init__(self,
        ...
        self.append_first_image = append_first_image
    ...
    def forward(self, rollout_batch, n_steps=None, variational=True):
        ...
        first_rollout = rollout_batch[:,0 ,:].clone().detach().requires_grad_(False)
        rollout_batch = conversions.concat_rgb(rollout_batch)
        #rollout_batch = rollout_batch.permute(0, 2, 1, 3, 4)  # (N,D,C,H,W) -> (N,C,D,H,W)

        if not self.structure_encoder==None:
            zstructure, zstrucutre_mean, zstructure_logvar = self.structure_encoder(rollout_batch, sample=variational)
            prediction.set_zs(zs_sample=zstructure, zs_mean=zstrucutre_mean, zs_logvar=zstructure_logvar)
        ...
        x_reconstructed = self.decoder(q, first_rollout)
        size = x_reconstructed.size()
    ...
    def load(self, directory):
        ...
        # TODO: Load structure encoder from file
    ...
    def save(self, directory):
        ...
        # TODO: Save structure encoder to file
